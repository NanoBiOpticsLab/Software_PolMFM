...
options = optimset('Jacobian','off','Display','off','MaxIter',10000); % options = optimset('Algorithm',{'levenberg-marquardt',0.005},'display','off');
...
%= muX-muY as fixed parameters - free parameters are muZ, sigma x-y-z, amp and bgd
elseif strcmp(fit_mode,'muXY-fixed_Z-and-sigma-free')
 ...
 %- Initial conditions
    ...
    %- Model parameters
    par_mod{1} = 5;     % Flag to indicate that muX&muY are fixed and that sigma_x = sigma_y 
    par_mod{2} = xdata;         
    par_mod{3} = pixel_size;
    par_mod{4} = options_fit.mu_X + dim.X*pixel_size.xy; 
    par_mod{5} = options_fit.mu_Y + dim.Y*pixel_size.xy;
    ...
    result.muX         = par_mod{4} - dim.X*pixel_size.xy;           % Center X
    result.muY         = par_mod{5} - dim.Y*pixel_size.xy;           % Center Y
    result.muZ         = x(3)       - dim.Z*pixel_size.z;           % Center Z
    result.amp         = x(4);           % Amplitude
    result.bgd         = x(5);           % Background
