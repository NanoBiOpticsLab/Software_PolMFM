...
if N_Spots
    ...
    if (flag_struct.parallel)
        ...
        for k=1:N_Spots
            ...
            shift_y = max(spots_detected(k,1)-parameters.box_spots_mosaic.xy,1);
            shift_x = max(spots_detected(k,2)-parameters.box_spots_mosaic.xy,1);
            shift_z = max(spots_detected(k,3)-parameters.box_spots_mosaic.z,1);

            if strcmp(mode_fit,'muXY-fixed_Z-and-sigma-free')
                options_fit.mu_X = parameters.muX(k) - (shift_x-1) * pixel_size.xy;
                options_fit.mu_Y = parameters.muY(k) - (shift_y-1) * pixel_size.xy;
            end
            if parameters.Photons_convert == 1
                sub_spots_k = (parameters.Cam_sensitivity/parameters.Cam_EM)*(sub_spots{k});
            else
                sub_spots_k = sub_spots{k};
            end
            FIT_Result{k} = psf_fit_3d_v7(sub_spots_k,[],options_fit,flag_struct);
