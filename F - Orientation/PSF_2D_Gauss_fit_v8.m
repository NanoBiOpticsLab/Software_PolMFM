function [PSF_fit, img_PSF] = PSF_2D_Gauss_fit_v8(img_PSF,parameters)
% Function to fit 2D image data to 2d Gaussian
...
[img_PSF.max.Y,img_PSF.max.X] = ind2sub(size(img_PSF.data),img_PSF.max.ind_lin);


%== Calculate theoretical PSF
[sigma_xy ~] = sigma_PSF_BoZhang_v1(par_microscope);
...
if flags.crop
    ...
    img_PSF.crop = double(img_PSF.data(y_min:y_max,x_min:x_max));     
...
[dim_sub.Y dim_sub.X] = size(img_PSF.crop);
N_pix                           = dim_sub.X*dim_sub.Y;

[Xs,Ys] = meshgrid(1:dim_sub.X,1:dim_sub.Y);
...
%- Perform fit
FIT_Result = psf_fit_2d_v7(img_PSF.crop,xdata,options_fit,flag_struct);
...
PSF_fit.fit_lin = fun_Gaussian_2D_v2(par_fit,par_mod);
