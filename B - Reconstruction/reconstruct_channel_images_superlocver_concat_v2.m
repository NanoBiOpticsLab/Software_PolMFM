%code to reconstruct and correct for intensity, of stack of images in batch
%mode
...
function [border,fullname_fr1,fullpath_reco,FileName] = reconstruct_channel_images_superlocver_concat_v2(path_calib,path_data,path_calib_int,nb_planes)

    [FileName_transfo, PathName_transfo]=uigetfile('*.mat','Select the transformation matrix',path_calib);
    calibrationFile=[PathName_transfo,FileName_transfo];

    [FileName_intcorr, PathName_intcorr]=uigetfile('*.mat','Select the intensity correction matrix',path_calib_int);
    correctionfactorFile=[PathName_intcorr,FileName_intcorr];
    
    [FileName, PathName]=uigetfile('*.tif','Select all the files to reconstruct',path_data,'MultiSelect','on');
...
%     [~,idxmin]=min(abs(diff_filter_em));
    ...
    nb_files = size(imFilename,2);

    % initialize total number of frames
    numFrames_tot = 0;
    for i_file=1:length(FileName)
        numFrames_tot=numFrames_tot+length(imfinfo(imFilename{i_file}));
    end
...
% init name of the reconstructed images for frame #1 
    nb_digits=numel(num2str(numFrames_tot)); 
    fullname_fr1 = [Name '_' num2str(1,['%.' num2str(nb_digits) 'd']) '.tif'];

    i_frame = 0; % initialize frame index out of total number of frames
    for i_file=1:length(FileName)
        info=imfinfo(imFilename{i_file});
        numFrames=length(info);
        for ff=1:numFrames
            i_frame = i_frame+1;
            im=imread(imFilename{i_file},'Index',ff);
            writeFilename_ff = fullfile(fullpath_reco,[Name '_' num2str(i_frame,['%.' num2str(nb_digits) 'd']) '.tif']);
            for kk=1:nb_planes
            ...
            perc=i_frame/numFrames_tot*100;
            ...
            %averreadnoise=[95,95,95,95,95,95,95,95,95];
            averagedark=[0,0,0,0,0,0];
            C = zeros(size(stack));
            C_noBckdRem = zeros(size(stack));
            ij=0;
            for k=[1 4 2 5 3 6]% 1:nb_planes %
                ij = ij+1;
               
                C(:,:,ij)=((stack(:,:,k)-averreadnoise(k)-averagedark(k))*corr(k));
                C_noBckdRem(:,:,ij)=((stack(:,:,k))*corr(k));
                if k==1
    ...
    border_value_name =[fullpath_reco,'\' ,FileName{1}(1:end-4), '_batch_border_value.mat']
